...
#include "imgui.h" // for ui::GetCurrentWindow()
...
{
	ImVec2 origin = ui::GetCursorScreenPos();
	...
	if (lineNo >= 0 && lineNo < (int)mLines.size())
	{
		...
		while ((size_t)columnIndex < line.size())
		{
			...
			{
				float spaceSize = ui::GetFont()->CalcTextSizeA(ui::GetFontSize(), FLT_MAX, -1.0f, " ").x;
				float oldX = columnX;
			...
			else
			{
				...
				buf[i] = '\0';
				columnWidth = ui::GetFont()->CalcTextSizeA(ui::GetFontSize(), FLT_MAX, -1.0f, buf).x;
				if (mTextStart + columnX + columnWidth * 0.5f > local.x)
					break;
				columnX += columnWidth;
				columnCoord++;
...
void TextEditor::HandleKeyboardInputs()
{
	...
	auto wtf = ctrl && ui::IsKeyPressed(ui::GetKeyIndex(ImGuiKey_Backspace));

	if (ui::IsWindowFocused())
	{
		if (ui::IsWindowHovered())
			ui::SetMouseCursor(ImGuiMouseCursor_TextInput);
		//ui::CaptureKeyboardFromApp(true);

		io.WantCaptureKeyboard = true;
		io.WantTextInput = true;

		if (!IsReadOnly() && ctrl && !shift && !alt && ui::IsKeyPressed(ui::GetKeyIndex(ImGuiKey_Z)))
			Undo();
		else if (!IsReadOnly() && !ctrl && !shift && alt && ui::IsKeyPressed(ui::GetKeyIndex(ImGuiKey_Backspace)))
			Undo();
		else if (!IsReadOnly() && ctrl && !shift && !alt && ui::IsKeyPressed(ui::GetKeyIndex(ImGuiKey_Y)))
			Redo();
		else if (!ctrl && !alt && ui::IsKeyPressed(ui::GetKeyIndex(ImGuiKey_UpArrow)))
			MoveUp(1, shift);
		else if (!ctrl && !alt && ui::IsKeyPressed(ui::GetKeyIndex(ImGuiKey_DownArrow)))
			MoveDown(1, shift);
		else if (!alt && ui::IsKeyPressed(ui::GetKeyIndex(ImGuiKey_LeftArrow)))
			MoveLeft(1, shift, ctrl);
		else if (!alt && ui::IsKeyPressed(ui::GetKeyIndex(ImGuiKey_RightArrow)))
			MoveRight(1, shift, ctrl);
		else if (!alt && ui::IsKeyPressed(ui::GetKeyIndex(ImGuiKey_PageUp)))
			MoveUp(GetPageSize() - 4, shift);
		else if (!alt && ui::IsKeyPressed(ui::GetKeyIndex(ImGuiKey_PageDown)))
			MoveDown(GetPageSize() - 4, shift);
		else if (!alt && ctrl && ui::IsKeyPressed(ui::GetKeyIndex(ImGuiKey_Home)))
			MoveTop(shift);
		else if (ctrl && !alt && ui::IsKeyPressed(ui::GetKeyIndex(ImGuiKey_End)))
			MoveBottom(shift);
		else if (!ctrl && !alt && ui::IsKeyPressed(ui::GetKeyIndex(ImGuiKey_Home)))
			MoveHome(shift);
		else if (!ctrl && !alt && ui::IsKeyPressed(ui::GetKeyIndex(ImGuiKey_End)))
			MoveEnd(shift);
		else if (!IsReadOnly() && !ctrl && !shift && !alt && ui::IsKeyPressed(ui::GetKeyIndex(ImGuiKey_Delete)))
			Delete();
		else if (!IsReadOnly() && !ctrl && !shift && !alt && ui::IsKeyPressed(ui::GetKeyIndex(ImGuiKey_Backspace)))
			Backspace();
		else if (ctrl && !shift && !alt && ui::IsKeyPressed(ui::GetKeyIndex(ImGuiKey_C)))
			Copy();
		else if (!IsReadOnly() && ctrl && !shift && !alt && ui::IsKeyPressed(ui::GetKeyIndex(ImGuiKey_V)))
			Paste();
		else if (ctrl && !shift && !alt && ui::IsKeyPressed(ui::GetKeyIndex(ImGuiKey_X)))
			Cut();
		else if (!ctrl && shift && !alt && ui::IsKeyPressed(ui::GetKeyIndex(ImGuiKey_Delete)))
			Cut();
		else if (ctrl && !shift && !alt && ui::IsKeyPressed(ui::GetKeyIndex(ImGuiKey_A)))
			SelectAll();
		else if (!IsReadOnly() && !ctrl && !shift && !alt && ui::IsKeyPressed(ui::GetKeyIndex(ImGuiKey_Enter)))
		{
			int tab_count = 0;
		...
		else if (!IsReadOnly() && !ctrl && !alt && ui::IsKeyPressed(ui::GetKeyIndex(ImGuiKey_Tab)))
...
void TextEditor::HandleMouseInputs()
{
	...
	if (ui::IsWindowHovered())
	{
		if (!shift && !alt)
		{
			auto click = ui::IsMouseClicked(0);
			auto doubleClick = ui::IsMouseDoubleClicked(0);
			auto t = ui::GetTime();
			...
			*/
			...
			if (tripleClick)
			{
				if (!ctrl)
				{
					mState.mCursorPosition = mInteractiveStart = mInteractiveEnd = ScreenPosToCoordinates(ui::GetMousePos());
			...
			*/
			...
			else if (ui::IsMouseDragging(0) && ui::IsMouseDown(0))
			{
				io.WantCaptureMouse = true;
				mState.mCursorPosition = mInteractiveEnd = ScreenPosToCoordinates(ui::GetMousePos());
				SetSelection(mInteractiveStart, mInteractiveEnd, mSelectionMode);
			}
		}
	}
...
void TextEditor::Render()
{
	...
	const float fontSize = ui::GetFont()->CalcTextSizeA(ui::GetFontSize(), FLT_MAX, -1.0f, "#", nullptr, nullptr).x;
	mCharAdvance = ImVec2(fontSize, ui::GetTextLineHeightWithSpacing() * mLineSpacing);

	/* Update palette with the current alpha from style */
	for (int i = 0; i < (int)PaletteIndex::Max; ++i)
	{
		auto color = ui::ColorConvertU32ToFloat4(mPaletteBase[i]);
		color.w *= ui::GetStyle().Alpha;
		mPalette[i] = ui::ColorConvertFloat4ToU32(color);
	}

	assert(mLineBuffer.empty());

	auto contentSize = ui::GetWindowContentRegionMax();
	...
	ImVec2 cursorScreenPos = ui::GetCursorScreenPos();
	auto scrollX = ui::GetScrollX();
	auto scrollY = ui::GetScrollY();
	...
	mTextStart = ui::GetFont()->CalcTextSizeA(ui::GetFontSize(), FLT_MAX, -1.0f, buf, nullptr, nullptr).x + mLeftMargin;

	if (!mLines.empty())
	{
		float spaceSize = ui::GetFont()->CalcTextSizeA(ui::GetFontSize(), FLT_MAX, -1.0f, " ", nullptr, nullptr).x;
		...
		while (lineNo <= lineMax)
		{
			...
			if (errorIt != mErrorMarkers.end())
			{
				...
				if (ui::IsMouseHoveringRect(lineStartScreenPos, end))
				{
					ui::BeginTooltip();
					ui::PushStyleColor(ImGuiCol_Text, ImVec4(1.0f, 0.2f, 0.2f, 1.0f));
					ui::Text("Error at line %d:", errorIt->first);
					ui::PopStyleColor();
					ui::Separator();
					ui::PushStyleColor(ImGuiCol_Text, ImVec4(1.0f, 1.0f, 0.2f, 1.0f));
					ui::Text("%s", errorIt->second.c_str());
					ui::PopStyleColor();
					ui::EndTooltip();
				}
			}
			...
			auto lineNoWidth = ui::GetFont()->CalcTextSizeA(ui::GetFontSize(), FLT_MAX, -1.0f, buf, nullptr, nullptr).x;
			drawList->AddText(ImVec2(lineStartScreenPos.x + mTextStart - lineNoWidth, lineStartScreenPos.y), mPalette[(int)PaletteIndex::LineNumber], buf);

			if (mState.mCursorPosition.mLine == lineNo)
			{
				auto focused = ui::IsWindowFocused();
					...
					if (elapsed > 400)
					{
						...
						if (mOverwrite && cindex < (int)line.size())
						{
							...
							else
							{
								...
								buf2[1] = '\0';
								width = ui::GetFont()->CalcTextSizeA(ui::GetFontSize(), FLT_MAX, -1.0f, buf2).x;
							}
						}
			...
			for (int i = 0; i < line.size();)
			{
				...
				if ((color != prevColor || glyph.mChar == '\t' || glyph.mChar == ' ') && !mLineBuffer.empty())
				{
					...
					auto textSize = ui::GetFont()->CalcTextSizeA(ui::GetFontSize(), FLT_MAX, -1.0f, mLineBuffer.c_str(), nullptr, nullptr);
		...
		{
			auto id = GetWordAt(ScreenPosToCoordinates(ui::GetMousePos()));
			if (!id.empty())
			{
				...
				{
					ui::BeginTooltip();
					ui::TextUnformatted(it->second.mDeclaration.c_str());
					ui::EndTooltip();
				}
				else
				{
					...
					{
						ui::BeginTooltip();
						ui::TextUnformatted(pi->second.mDeclaration.c_str());
						ui::EndTooltip();
					}
				}
			}
	...
	ui::Dummy(ImVec2((longest + 2), mLines.size() * mCharAdvance.y));
...
void TextEditor::Render(const char* aTitle, const ImVec2& aSize, bool aBorder)
{
	...
	mPalette[(int)PaletteIndex::Background] = ui::ColorConvertFloat4ToU32(ui::GetStyle().Colors[ImGuiCol_ChildBg]);
	ui::PushStyleVar(ImGuiStyleVar_ItemSpacing, ImVec2(0.0f, 0.0f));
	if (!mIgnoreImGuiChild)
		ui::BeginChild(aTitle, aSize, aBorder, ImGuiWindowFlags_HorizontalScrollbar | ImGuiWindowFlags_NoMove);

	if (mHandleKeyboardInputs)
	{
		HandleKeyboardInputs();
		ui::PushAllowKeyboardFocus(true);
	}
...
void TextEditor::Copy()
{
	if (HasSelection())
	{
		ui::SetClipboardText(GetSelectedText().c_str());
	}
	else
	{
		if (!mLines.empty())
...
void TextEditor::Paste()
{
	...
	auto clipText = ui::GetClipboardText();
...
float TextEditor::TextDistanceToLineStart(const Coordinates& aFrom) const
{
	...
	for (size_t it = 0u; it < line.size() && it < colIndex; )
	{
		...
		else
		{
			...
			tempCString[i] = '\0';
			distance += ui::GetFont()->CalcTextSizeA(ui::GetFontSize(), FLT_MAX, -1.0f, tempCString, nullptr, nullptr).x;
		}
	}
...
void TextEditor::EnsureCursorVisible()
{
	...
	float scrollX = ui::GetScrollX();
	float scrollY = ui::GetScrollY();

	auto height = ui::GetWindowHeight();
	auto width = ui::GetWindowWidth();
	...
	if (pos.mLine < top)
		ui::SetScrollY(max(0.0f, (pos.mLine - 1) * mCharAdvance.y));
	if (pos.mLine > bottom - 4)
		ui::SetScrollY(max(0.0f, (pos.mLine + 4) * mCharAdvance.y - height));
	if (len + mTextStart < left + 4)
		ui::SetScrollX(max(0.0f, len + mTextStart - 4));
	if (len + mTextStart > right - 4)
		ui::SetScrollX(max(0.0f, len + mTextStart + 4 - width));
}

int TextEditor::GetPageSize() const
{
	auto height = ui::GetWindowHeight() - 20.0f;
