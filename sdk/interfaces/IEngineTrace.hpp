...
public:
    ...
    inline int get_entity_index() const { return hit_entity ? hit_entity->EntIndex() : -1; }
    inline bool did_hit_world() const { return get_entity_index() == 0; }
    inline bool did_hit_non_world_entity() const { return hit_entity && !did_hit_world(); }
public:
...
public:
    ...
    inline void trace_hull(Vector& src, Vector& end, std::uint32_t mask, trace_t& tr)
    {
        Ray_t ray;
        ...
        TraceRay(ray, mask, &filter, &tr);
    }

    inline void trace_line(const Vector& vecAbsStart, const Vector& vecAbsEnd, unsigned int mask, trace_t* ptr)
    {
        Ray_t ray;
