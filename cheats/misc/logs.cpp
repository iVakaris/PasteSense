...
#if RELEASE
#if BETA
		m_cvar()->ConsoleColorPrintf(g_cfg.misc.log_color, crypt_str("[ gamesense beta ] ")); //-V807
#else
		m_cvar()->ConsoleColorPrintf(g_cfg.misc.log_color, crypt_str("[ gamesense ] "));
#endif
#else
...
#if RELEASE
#if BETA
		auto log = crypt_str("[ \x0Ccgamesense beta \x01] ") + text;
		chat->chat_print(log.c_str());
#else
		auto log = crypt_str("[ \x0Ccgamesense \x01] ") + text;
		chat->chat_print(log.c_str());
#endif
#else
		auto log = crypt_str("[ \x0Ccgamesense \x01] ") + text;
