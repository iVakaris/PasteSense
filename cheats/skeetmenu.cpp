...
void menu::draw()
{
	...
	if (this->m_nCurrentTab == 0)
	{
		...
		ui::BeginChildSkeet("WEAPON SETTINGS RAGE", ImVec2(child_size.x, child_size.y + 200), 0);
		{
			...
			{
				ui::SliderFloatSkeet(crypt_str("Head scale"), &g_cfg.ragebot.weapon[hooks::rage_weapon].head_scale, 0.0f, 1.0f, g_cfg.ragebot.weapon[hooks::rage_weapon].head_scale ? crypt_str("%.1f") : crypt_str("None"));
				ui::SliderFloatSkeet(crypt_str("Body scale"), &g_cfg.ragebot.weapon[hooks::rage_weapon].body_scale, 0.0f, 1.0f, g_cfg.ragebot.weapon[hooks::rage_weapon].body_scale ? crypt_str("%.1f") : crypt_str("None"));
			}
	...
	if (this->m_nCurrentTab == 4)
	{
		...
		ui::BeginChildSkeet("MISC MAIN", child_size);
		{
			...
			ui::CheckboxSkeet(crypt_str("Clantag Pastesense"), &g_cfg.misc.clantag_spammer_pastesense);
			ui::CheckboxSkeet(crypt_str("Clantag Unlucky"), &g_cfg.misc.clantag_spammer_unlucky);
	...
	ui::PopFont();
	auto fx = ui::CalcTextSize(("Shonax"));
	ui::EndSkeet();
	...
	ui::BeginSkeet("shonaxz", 0, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoResize);
	if (g_DrawModel->GetTexture())
	{
		...
		{
			RenderText("Shonax", ImVec2(vecWindowPosition.x + 715 + 150 + 80 - 50 / 2, vecWindowPosition.y + 22), 15.f, aName, false, false, skeet_menu.f2);
		}
